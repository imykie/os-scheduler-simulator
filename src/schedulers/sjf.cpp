...
SJF::SJF(Timestamp *timer, Queue<Process *> *processes) : TimestampObserver(timer) {
    this->timer = timer;
...
void SJF::Simulate() {
    ...
    while (terminated_queue->Length() < process_count) {
        ...
        // handle job queue
        while (!job_queue->IsEmpty() && job_queue->Peek()->GetArrivalTime() <= current_time) {
        ...
        if (!ready_queue->IsEmpty() && current_process == nullptr) {
            ready_queue = SortQueue(ready_queue);
...
bool SJF::SortFunction(Process *p1, Process *p2) {
    return p1->GetCPUBurstTime1() + p1->GetCPUBurstTime2() <= p2->GetCPUBurstTime1() + p2->GetCPUBurstTime2();
}

Queue<Process *> *SJF::SortQueue(Queue<Process *> *queue) {
    if (queue->Length() > 0) {
        return QueueUtils::Sort(queue, SortFunction);
    }
    return queue;
}