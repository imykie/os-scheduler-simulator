...
std::string Process::ToString()
{
    ...
    out.push_back("[CPU Burst Time 1]: " + std::to_string(this->CPU_burst_time_1));
    out.push_back("[CPU Burst Time 2]: " + std::to_string(this->CPU_burst_time_2));
    out.push_back("[IO Burst Time]: " + std::to_string(this->IO_burst_time));
    out.push_back("[Response Time]: " + std::to_string(this->response_time));
    out.push_back("[Turn Around Time]: " + std::to_string(this->turn_around_time));
    out.push_back("[Waiting Time]: " + std::to_string(this->waiting_time));
    out.push_back("[Termination Time]: " + std::to_string(this->termination_time));

    const char *const delim = "\n";
    std::ostringstream imploded;
    std::copy(out.begin(), out.end(), std::ostream_iterator<std::string>(imploded, delim));
    return imploded.str();
};
...
unsigned int Process::GetCPUBurstTime1() const { return this->CPU_burst_time_1; };
unsigned int Process::GetCPUBurstTime2() const { return this->CPU_burst_time_2; };
...
// Setters
void Process::SetProcessID(unsigned int process_id) { this->process_id = process_id; };
void Process::SetArrivalTime(unsigned int arrival_time) { this->arrival_time = arrival_time; };
void Process::SetCPUBurstTime1(unsigned int CPU_burst_time_1) { this->CPU_burst_time_1 = CPU_burst_time_1; };
void Process::SetCPUBurstTime2(unsigned int CPU_burst_time_2) { this->CPU_burst_time_2 = CPU_burst_time_2; };
void Process::SetIOBurstTime(unsigned int IO_burst_time) { this->IO_burst_time = IO_burst_time; };
void Process::SetResponseTime(int response_time) { this->response_time = response_time; };
void Process::SetTurnAroundTime(int turn_around_time) { this->turn_around_time = turn_around_time; };
void Process::SetWaitTime(int waiting_time) { this->waiting_time = waiting_time; };
void Process::SetTerminationTime(int termination_time) { this->termination_time = termination_time; };
